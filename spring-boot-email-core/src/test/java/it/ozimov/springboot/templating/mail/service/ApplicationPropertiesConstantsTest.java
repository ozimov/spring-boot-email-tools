...
public class ApplicationPropertiesConstantsTest {
    ...
    public void shouldConstantsRemainUnchanged() {
        ...
        assertions.assertThat(SPRING_MAIL_SCHEDULER_PERSISTENCE_LAYER_DESIRED_BATCH_SIZE).isEqualTo("spring.mail.scheduler.persistenceLayer.desiredBatchSize");
        assertions.assertThat(SPRING_MAIL_SCHEDULER_PERSISTENCE_LAYER_MIN_KEPT_IN_MEMORY).isEqualTo("spring.mail.scheduler.persistenceLayer.minKeptInMemory");
        assertions.assertThat(SPRING_MAIL_SCHEDULER_PERSISTENCE_LAYER_MAX_KEPT_IN_MEMORY).isEqualTo("spring.mail.scheduler.persistenceLayer.maxKeptInMemory");
        assertions.assertThat(SPRING_MAIL_PERSISTENCE_ENABLED).isEqualTo("spring.mail.persistence.enabled");
        assertions.assertThat(SPRING_MAIL_PERSISTENCE_REDIS_ENABLED).isEqualTo("spring.mail.persistence.redis.enabled");
        assertions.assertThat(SPRING_MAIL_PERSISTENCE_REDIS_EMBEDDED).isEqualTo("spring.mail.persistence.redis.embedded");
        assertions.assertThat(SPRING_MAIL_PERSISTENCE_REDIS_HOST).isEqualTo("spring.mail.persistence.redis.host");
        assertions.assertThat(SPRING_MAIL_PERSISTENCE_REDIS_PORT).isEqualTo("spring.mail.persistence.redis.port");
    }
